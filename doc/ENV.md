...
| `ROUTES_PATH` | `routes.json` | Non-empty file path | Optional manual routing file. If the file does not exist or validation fails, Vectis falls back to `FINAL_APP_ADDR` and `FINAL_APP_PATH`. |
...
These variables control defaults for operational keys created by `POST /keys`. They do not change the fixed internal key material generated by `cli init`.
