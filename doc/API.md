...
`VECTIS_APIKEY` is the client secret sent in the `X-API-Key` header. The server validates it against `VECTIS_APIKEY_HASH`, which is generated by `vectis init` or `vectis apikey create` and derived with the init API auth key.
