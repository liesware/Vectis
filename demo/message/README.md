...
The scripts create local demo state under `demo/message/site-a` and
`demo/message/site-b`. This state includes encrypted `init.json`,
`.unseal_key`, SQLite storage, and a unified
...
- The generated `.unseal_key`, `.env`, databases, and route signatures are demo
  state and are ignored by `demo/message/.gitignore`.
